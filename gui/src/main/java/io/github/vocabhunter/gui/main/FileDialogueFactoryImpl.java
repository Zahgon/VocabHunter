...
public class FileDialogueFactoryImpl implements FileDialogueFactory {
    ...
    private FileDialogue createNewSessionDialogue(final Window window) {
        return new FileDialogueImpl(i18nManager, OPEN_SESSION, window, settingsManager, TYPES_INPUT_DOCUMENTS, FileChooser::showOpenDialog, SettingsManager::getDocumentsPath, SettingsManager::setDocumentsPath);
    }

    private FileDialogue createOpenSessionDialogue(final Window window) {
        return new FileDialogueImpl(i18nManager, OPEN_SESSION, window, settingsManager, TYPES_SESSIONS, FileChooser::showOpenDialog, SettingsManager::getSessionsPath, SettingsManager::setSessionsPath);
    }

    private FileDialogue createSaveSessionDialogue(final Window window) {
        return new FileDialogueImpl(i18nManager, SAVE_SESSION, window, settingsManager, TYPES_SESSIONS, FileChooser::showSaveDialog, SettingsManager::getSessionsPath, SettingsManager::setSessionsPath);
    }

    private FileDialogue createExportSelectionDialogue(final Window window) {
        return new FileDialogueImpl(i18nManager, EXPORT_SELECTION, window, settingsManager, TYPES_EXPORTS, FileChooser::showSaveDialog, SettingsManager::getExportPath, SettingsManager::setExportPath);
    }

    private FileDialogue createOpenWordListDialogue(final Window window) {
        return new FileDialogueImpl(i18nManager, OPEN_WORD_LIST, window, settingsManager, TYPES_WORD_GRIDS, FileChooser::showOpenDialog, SettingsManager::getWordListPath, SettingsManager::setWordListPath);
    }

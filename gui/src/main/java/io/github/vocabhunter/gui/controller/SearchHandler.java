...
public class SearchHandler {
    ...
    public SearchHandler(final GuiTaskHandler guiTaskHandler, final I18nManager i18nManager, final WordListHandler wordListHandler, final SearchControls controls, final SessionModel model) {
    ...
    private void processTextUpdate(final String value) {
        if (StringUtils.isNotBlank(value)) {
            ...
            model.getWordList().stream().filter(matcher).findFirst().ifPresent(wordListHandler::selectWord);
        }
    }

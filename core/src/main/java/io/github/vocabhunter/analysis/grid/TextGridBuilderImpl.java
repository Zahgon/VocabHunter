...
public class TextGridBuilderImpl implements TextGridBuilder {
    ...
    private List<GridLine> normalise(final List<GridLine> lines) {
        List<GridLine> normalised = lines.stream().map(this::normalise).toList();
        int last = CoreTool.findLast(normalised, l -> !l.getCells().isEmpty()).orElse(-1) + 1;
    ...
    private GridLine normalise(final GridLine input) {
        ...
        int newCount = CoreTool.findLast(cells, c -> !c.isEmpty()).orElse(-1) + 1;

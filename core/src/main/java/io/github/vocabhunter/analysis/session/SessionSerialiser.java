...
public final class SessionSerialiser {
    ...
    private static SessionState upgradeVersion1And2(final SessionState original) {
        ...
        List<SessionWord> words = original.getOrderedUses().stream().map(SessionSerialiser::upgradeVersion1And2).sorted(WordStreamTool.WORD_COMPARATOR).toList();
    ...
    private static SessionState upgradeVersion3(final SessionState original) {
        ...
        List<SessionWord> words = original.getOrderedUses().stream().map(w -> upgradeVersion3(tool, w)).toList();
    ...
    private static SessionWord upgradeVersion3(final LineListTool<SessionWord> tool, final SessionWord original) {
        List<Integer> lineNos = original.getUses().stream().map(tool::getLineNo).toList();
        SessionWord word = new SessionWord();
    ...
    private static int countWords(final String identifier, final List<String> uses) {
        return (int) uses.stream().flatMap(WordStreamTool::words).filter(w -> w.equalsIgnoreCase(identifier)).count();
    }

    private static String identifier(final String oldIdentifier, final List<String> uses) {
        return uses.stream().flatMap(WordStreamTool::words).filter(w -> w.equalsIgnoreCase(oldIdentifier)).reduce(PreferredFormTool::preferredForm).orElse(oldIdentifier);
    }
